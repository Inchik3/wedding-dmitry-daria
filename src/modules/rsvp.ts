...
import { validateRsvp, type Attendance, type RsvpFormData, type RsvpPayload } from './validation';
...
export function initRsvp(): void {
  ...
  const syncDrinks = () => {
    ...
    drinksGroup.hidden = declined;
    drinksGroup.querySelectorAll<HTMLInputElement>('input').forEach((input) => {
      if (declined) input.checked = false;
    });
  };

  attendanceInputs.forEach((input) => input.addEventListener('change', syncDrinks));
  syncDrinks();
  ...
  form.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      status.textContent = response.message || 'Спасибо! Ваш ответ сохранён.';
      status.dataset.state = 'success';
      form.reset();
      syncDrinks();
    } catch {
      status.textContent = 'Не удалось отправить ответ. Проверьте соединение и попробуйте ещё раз.';
